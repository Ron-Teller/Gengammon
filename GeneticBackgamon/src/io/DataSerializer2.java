...
public class DataSerializer2 {

	public List<BackgammonGene> loadPopulation(String file) {
		List<BackgammonGene> population = new ArrayList<BackgammonGene>();
		BackgammonGene gene;
		try {
		    FileInputStream in = new FileInputStream(file);
		    ObjectInputStream ois = new ObjectInputStream(in);
		    while (in.available() > 0) {
		    	gene = (BackgammonGene) ois.readObject();
		    	population.add(gene);
		    }
		    ois.close();
		  } catch (Exception e) {
	...
	public void savePopulation(List<BackgammonGene> population, String file) {
		try {
		      FileOutputStream out = new FileOutputStream(file);
		      ObjectOutputStream oos = new ObjectOutputStream(out);
		      for (BackgammonGene gene : population) {
